...
# ============================================================
# This is the missing layer above CardGameBanker (draw/stay thresholds):
# Hoffman's "thinksets" decide which trump to use (Curse, Shield Assault, Mind Shift, etc.)
# before the banker chooses draw/stay. We approximate this with a data-driven rule table.
#
# You can refine the placeholders by inspecting:
...
# and the thinkset modules in:
#   chapter7_2/survival/hoffmans/*.fsm.16

HOFFMAN_TRUMP_ALIASES = {
    # Normalize plus-variants to base keys
...
def infer_hoffman_thinkset(intel: dict) -> str:
    """
    Best-effort mapping from opponent + trump kit → thinkset name used by the Hoffman FSMs.
    This is *good enough* to start recreating behavior, and you can hard-map per opponent later.
    """
    ...
    # Molded / curse package
    if {"Curse", "Black Magic", "Conjure"} & trumps or "Molded" in name:
        return "Molded"

    # Shield Assault package
    if "Shield Assault" in trumps:
        return "Defence"

    # Mind Shift / Desire / Happiness package
    if {"Mind Shift", "Desire", "Happiness"} & trumps:
        # Survival+ tends to use the _p modules, but we keep names stable in this helper.
        return "Tricky"

    # Default: basic tally-mark behavior (pressure with bet-ups)
    return "Attacker"
...
def predict_enemy_trump(intel: dict, condition_state: "TrumpConditionState") -> tuple:
    ...
    Combines:
      1) CardGameCondition-style likelihoods (your existing engine)
      2) Hoffman thinkset rule priorities (new layer)
    """
    # Candidate set = opponent special + standard trumps
    enemy_trumps = [_norm_trump(t) for t in (intel.get("trumps", []) + intel.get("standard_trumps", []))]
    if not enemy_trumps or condition_state is None:
        return (None, "LOW", "No enemy trumps known / no condition state")

    thinkset = infer_hoffman_thinkset(intel)

    bhs = getattr(condition_state, "banker_hand_sum", 0)
    phs = getattr(condition_state, "player_hand_sum", 0)
    r   = getattr(condition_state, "round", 1)

    # --- Hoffman “rule” layer (fast approximations from hoffmans/*.fsm strings) ---
    # If multiple rules match, we pick the highest priority (lowest number).
    hoffman_pick = None
    hoffman_reason = ""
    # Attacker: pressure with bet-ups if behind / mid totals
    if thinkset == "Attacker":
        if "One-Up" in enemy_trumps and (bhs < phs or bhs <= 19):
            hoffman_pick = "One-Up"
            hoffman_reason = f"{thinkset}: pressure bet (banker {bhs} vs you {phs})"
        elif "Two-Up" in enemy_trumps and (bhs < phs - 2):
            hoffman_pick = "Two-Up"
            hoffman_reason = f"{thinkset}: spike bet when behind (Δ={phs - bhs})"

    # Defence: react to bet pressure + use Shield Assault
    if thinkset == "Defence":
        if "Shield Assault" in enemy_trumps and phs >= 17 and bhs <= 18:
            hoffman_pick = "Shield Assault"
            hoffman_reason = f"{thinkset}: punish strong player total ({phs})"
        elif "Go for 17" in enemy_trumps and 15 <= bhs <= 17:
            hoffman_pick = "Go for 17"
            hoffman_reason = f"{thinkset}: safer target (banker in 15–17 band)"

    # Tricky: disruptive cards in 19–21 band + trump-count punish
    if thinkset == "Tricky":
        if "Mind Shift" in enemy_trumps and r >= 1 and getattr(condition_state, "player_item_num", 0) >= 4:
            hoffman_pick = "Mind Shift"
            hoffman_reason = f"{thinkset}: you’re hoarding trumps (you have {condition_state.player_item_num})"
        elif "Desire" in enemy_trumps and getattr(condition_state, "player_item_num", 0) >= 3:
            hoffman_pick = "Desire"
            hoffman_reason = f"{thinkset}: Desire scales with your trump count ({condition_state.player_item_num})"
        elif "Happiness" in enemy_trumps and 19 <= bhs <= 21:
            hoffman_pick = "Happiness"
            hoffman_reason = f"{thinkset}: edge-band disruption (banker {bhs})"

    # Molded: curse when banker is weak; conjure early when low on trumps
    if thinkset == "Molded":
        if "Curse" in enemy_trumps and (bhs <= 16 or (bhs <= 16 and phs >= 17)):
            hoffman_pick = "Curse"
            hoffman_reason = f"{thinkset}: banker weak ({bhs}) vs you ({phs})"
        elif "Conjure" in enemy_trumps and getattr(condition_state, "banker_item_num", 0) <= 1 and r <= 3:
            hoffman_pick = "Conjure"
            hoffman_reason = f"{thinkset}: refill trumps early (banker_item_num={condition_state.banker_item_num})"

    # LassBoss: perfect-draw midband; dead-silence/oblivion on edge
    if thinkset == "LassBoss":
        if "Perfect Draw" in enemy_trumps and 10 <= bhs <= 18:
            hoffman_pick = "Perfect Draw+"
            hoffman_reason = f"{thinkset}: perfect-draw in 10–18 band (banker {bhs})"
        elif "Dead Silence" in enemy_trumps and 19 <= bhs <= 21:
            hoffman_pick = "Dead Silence"
            hoffman_reason = f"{thinkset}: shuts down your options near finish"
        elif "Oblivion" in enemy_trumps and bhs < phs and r >= 2:
            hoffman_pick = "Oblivion"
            hoffman_reason = f"{thinkset}: erase losing round (banker {bhs} vs you {phs})"

    # Rare: Escape when threatened
    if thinkset == "Rare":
        if "Escape" in enemy_trumps and (bhs < phs and r >= 2):
            hoffman_pick = "Escape"
            hoffman_reason = f"{thinkset}: bailout when behind (Δ={phs - bhs})"

    # --- Condition engine layer (your existing per-trump likelihood) ---
    best_by_condition = None
    best_score = -1
    best_reason = ""
    for trump in set(enemy_trumps):
    ...
    # Combine: if Hoffman's pick is also high by condition, boost it; otherwise allow overrides
    if hoffman_pick:
        # Look up condition score for the hoffman pick if available
        hp = _norm_trump(hoffman_pick)
        lvl, rsn = condition_state.trump_fire_likelihood(hp) if hp in set(enemy_trumps) else ("LOW", "thinkset-driven")
        hscore = _level_to_score(lvl) + 35  # thinkset bias
        if hscore >= best_score:
            conf = "HIGH" if hscore >= 90 else ("MEDIUM" if hscore >= 55 else "LOW")
            return (hoffman_pick, conf, f"{hoffman_reason} | {hp} [{lvl}]: {rsn}")
        else:
            conf = "HIGH" if best_score >= 90 else ("MEDIUM" if best_score >= 55 else "LOW")
            return (best_by_condition, conf, best_reason + f" | (thinkset suggested {hoffman_pick}: {hoffman_reason})")
...
def generate_advice(
    ...
    # ── HOFFMAN THINKSET PREDICTION (enemy trump choice) ──
    if condition_state is not None:
        pick, conf, why = predict_enemy_trump(intel, condition_state)
        if pick:
            advice_lines.append(f"ENEMY AI PREDICT [{conf}]: likely plays '{pick}' next — {why}")
    if player_hp <= 3:
...
def fight_opponent(intel: dict, player_hp: int, player_max: int,
    ...
    while player_hp > 0 and opp_hp > 0:
        round_num += 1
        # Keep CardGameCondition-like fields fresh for timing predictions
        try:
            _condition.round = round_num
            _condition.kill_count = max(0, fight_num - 1)  # Survival+ kill counter proxy
            _condition.banker_item_num = len(intel.get("trumps", [])) + len(intel.get("standard_trumps", []))
            _condition.player_item_num = len(trump_hand) if trump_hand else 0
        except Exception:
            pass
